...
def fetch(proxy_list, filename):
    ...
    working = yaml.safe_load(requests.get(url=baseurl + current_date + '/' + filename, timeout=5000).text)
    data_out = []
...
def url(proxy_list, link):
    try:
        data_out=[]
        working = yaml.safe_load(requests.get(url=link, timeout=5000, headers=headers).text)
        for x in working['proxies']:
            data_out.append(x)
        proxy_list.append(data_out)
    except:

...
async function run(){
    ...
    for(let i=0;i<finalList.length;i++){
        ...
        switch (item.type){
            ...
            case 'ssr':
                try{
                urlCountryList[finalList[i].country].push('ssr://'+Buffer.from(item.data.replace('{name}',Buffer.from((name).toString(),'utf8').toString('base64').replace(/+/g, "-")),'utf8').toString('base64'))

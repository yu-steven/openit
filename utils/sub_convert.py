        ...
        for proxy in proxies_list: # 改名
            if format_name_enabled:
                ...
                if len(proxies_list) > 999:
                    proxy['name'] = f'倾城极速.tk-{name_emoji}{country_code}-Openit.ml'# {name_emoji}{country_code}-{ip}-{proxy_index:0>4d}
                elif len(proxies_list) < 999 and len(proxies_list) > 99:
                    proxy['name'] = f'倾城极速.tk-{name_emoji}{country_code}-Openit.ml'# {name_emoji}{country_code}-{ip}-{proxy_index:0>3d}
                elif len(proxies_list) < 99:
                    proxy['name'] = f'倾城极速.tk-{name_emoji}{country_code}-Openit.ml'# {name_emoji}{country_code}-{ip}-{proxy_index:0>2d}

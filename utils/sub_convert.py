...
class sub_convert():
    ...
    def makeup(input, dup_rm_enabled=True, format_name_enabled=True, speedtest=True): # 对节点进行区域的去重筛选和重命名以及测速，输出 YAML 文本 
        ...
        if speedtest: # 测速
            ...
            for proxy in proxies_list:
                ...
                if ping_result[0] >= 0.05: #and ping_result_g[0] >= 0.05 and ping_result_y[0] >= 0.05，要加回去，别忘了前面的冒号
                    proxies_list.remove(proxy)
                elif ping_result[1] < 1: #or ping_result_g[1] < 1 or ping_result_y[1] < 1，要加回去，别忘了前面的冒号
                    proxies_list.remove(proxy)
        ...
        for proxy in proxies_list: # 改名
            if format_name_enabled:
                ...
                if len(proxies_list) > 999:
                    proxy['name'] = f'{name_emoji}{country_code}-Openit.ml'# {name_emoji}{country_code}-{ip}-{proxy_index:0>4d}
                elif len(proxies_list) < 999 and len(proxies_list) > 99:
                    proxy['name'] = f'{name_emoji}{country_code}-Openit.ml'# {name_emoji}{country_code}-{ip}-{proxy_index:0>3d}
                elif len(proxies_list) < 99:
                    proxy['name'] = f'{name_emoji}{country_code}-Openit.ml'# {name_emoji}{country_code}-{ip}-{proxy_index:0>2d}
    ...
    def convert_remote(url='', output_type='clash', host='http://0.0.0.1:25500'): #{url='订阅链接', output_type={'clash': 输出 Clash 配置, 'base64': 输出 Base64 配置, 'url': 输出 url 配置}, host='远程订阅转化服务地址'}
        # 使用远程订阅转换服务，输出相应配置。
        sever_host = host
        url = urllib.parse.quote(url, safe='') # https://docs.python.org/zh-cn/3/library/urllib.parse.html
        #http://0.0.0.1:25500/sub?target=clash&url=url&insert=false&config=&emoji=true&list=true&udp=false&tfo=false&expand=true&scv=false&fdn=false&sort=false&new_name=true
